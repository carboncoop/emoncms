        ...
        if ($route->format == 'html') {
            if ($route->action == 'view')
                ...
                if ($route->subaction == 'backup' && $session['write'] && $session['admin']) {
                    ...
                    if (!$fh)
                        $result = "ERROR: Can't write the flag $backup_flag.";
                    else
            ...
            if ($route->action == 'redisflush' && $session['write']) {
                ...
                $data = array();
                $result = $mysqli->query("SELECT id,username,email,admin FROM users");
                ...
                if (isset($fields->admin)) {
                    ...
                    $stmt = $mysqli->prepare("UPDATE users SET admin = ? WHERE id = ?");
                    $stmt->bind_param("ii", $fields->admin, $id);
                    if ($stmt->execute())
                        $success = true;
                    $stmt->close();
                ...
                if ($success) {
                    $result = array('success' => true, 'message' => 'Field updated');
                }
                else {
                    $result = array('success' => false, 'message' => 'Field could not be updated');
                }
            }
        }
    }
